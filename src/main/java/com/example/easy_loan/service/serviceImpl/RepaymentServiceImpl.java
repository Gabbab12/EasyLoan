...
import com.example.easy_loan.api.InitiateTransfer.InitiateTransfer;
import com.example.easy_loan.api.verifyaccount.VerifyAccount;
import com.example.easy_loan.dto.CreateRecipientDto;
import com.example.easy_loan.dto.InitiateTransferDto;
import com.example.easy_loan.dto.TransferRequest;
...
import com.example.easy_loan.repository.*;
import com.example.easy_loan.service.RepaymentService;
...
public class RepaymentServiceImpl implements RepaymentService {
        ...
        return webClient.post()
                ...
                .map(responseEntity -> {
                    if (responseEntity.getStatusCode().is2xxSuccessful()) {
                        log.info("Transfer initiated successfully. Transfer ID: {}", Objects.requireNonNull(responseEntity.getBody()).getData());
    ...
    @Override
    public String verifyAccountDetailsAndSetup(CreateRecipientDto createRecipientDto) {
        ...
        if (verifiedAccount != null && verifiedAccount.getData() != null) {
            return verifiedAccount.getData().getAccountName();
        } else {
            throw new ResourceNotFoundException("Bank holder name not found");
        }
    }

    @Override
    public BankAccount saveVerifiedAccount(CreateRecipientDto createRecipientDto) {

        String bankHolderName = verifyAccountDetailsAndSetup(createRecipientDto);
...
//        // Fetch the user by username
        ...
        BankAccount bankAccount = BankAccount.builder()
                ...
                .accountName(bankHolderName)
                .user(user2)
                .build();

        // Save the BankAccount entity to the database
        System.out.println("External API Response: " + bankHolderName);
